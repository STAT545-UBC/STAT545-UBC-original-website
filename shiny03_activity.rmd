...
```{r define-simple-ui, echo=FALSE, eval=TRUE}
library(shiny)
ui <- fluidPage(
  titlePanel("BC Liquor Store prices"),
  sidebarLayout(
...
file <- tempfile()
writeLines(trimws(tmp), file)
```

```{r as.is = TRUE, echo = FALSE, comment = NA, eval=TRUE}
cat(readLines(file), sep = "\n")
invisible(suppressWarnings(file.remove(file)))
```
